...
def test_equivalent_multiplier_resolution_is_canonical() -> None:
    chosen = MethodOneMultiplierCandidate(
        MultiplierCategory.FORM, 1.0, MultiplierScope.MATCH, "m"
    )
    first = MethodOneMultiplierCandidate(
        MultiplierCategory.FORM, 1.0, MultiplierScope.GLOBAL, "z"
    )
    second = MethodOneMultiplierCandidate(
        MultiplierCategory.FORM, 1.0, MultiplierScope.COMPETITION, "a"
    )
    left = MethodOneMultiplierResolution(
        MultiplierCategory.FORM, chosen, (first, second)
    )
    right = MethodOneMultiplierResolution(
        MultiplierCategory.FORM, chosen, (second, first)
    )
    assert left == right
    assert left.discarded[0] is second
